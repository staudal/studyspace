    ...
    <li className='col-span-1 divide-y divide-gray-200 rounded-lg shadow' style={{ background: subject.color.hex }}>
      ...
      <div className='-mt-px flex divide-x divide-gray-200 bg-gray-50 rounded-b-lg'>
        {subject.pages.map((page) => (
          <div key={page.id} className='w-0 flex-1 flex'>
            <Link
              href={'/fag/' + page.slug}
              className='relative -mr-px inline-flex w-0 flex-1 items-center justify-center gap-x-3 rounded-bl-lg border border-transparent py-4 text-sm font-semibold text-gray-900 hover:text-blue-500'
            >
              Niveau {page.level}
            </Link>
          </div>
